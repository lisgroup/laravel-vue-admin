...
class CreateArticlesTable extends Migration {
	...
	public function up()
	{
		...
		{
			$table->increments('id')->comment('文章表 id');
			$table->boolean('category_id')->default(0)->comment('分类 id');
			$table->string('title')->default('')->comment('标题');
			$table->string('author')->default('')->comment('作者');
			$table->mediumText('markdown')->comment('markdown 内容');
			$table->mediumText('content')->comment('其它编辑器内容');
